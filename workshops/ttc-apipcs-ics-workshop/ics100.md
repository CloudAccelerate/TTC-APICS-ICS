...
- In the popup window, supply the following values:
    ...
    - **Select** options to configure `request payload`, and configure `endpoint response`
    - Also make sure `Configure Request Headers` to select `Custom`
...
Now we have to configure the tracking fields for the orchestration so that we can see what happened in the `Monitoring` tools.
...
- Make sure to select `Enable Tracking` and click on `Activate and Publish`
